...
export default function ProjectsPage() {
    ...
    return (
        <div className="p-8 pt-[9rem] bg-white dark:bg-black h-screen">
            ...
            <div className=" w-full max-w-7xl mx-auto grid grid-cols-[repeat(auto-fit,minmax(400px,1fr))] gap-6 place-items-center">
                ...
                {projects.map((p) => (
                    <div
                        key={p.id}
                        className="bg-white dark:bg-[#0A0A0A] border border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500 rounded-md md:w-[420px] w-96 p-4 text-black dark:text-white relative transition"
                    >
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                >
                                    {p.live_link?.replace("https://", "").replace("http://", "")}
                                </a>
                            </div>
                        ...
                        {/* Description (fallback if none in DB) */}
                        <p className="mt-3 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">
                            {p.description || "No description provided."}
                        </p>

                        {/* Footer info (mock commit info / branch) */}
